...
export function HomeHero() {
  return (
    <section className="relative pt-1 md:pt-4 flex flex-col items-center text-center space-y-2 md:space-y-4 px-4 overflow-hidden">
      ...
      <div className="absolute top-[-20%] left-1/2 -translate-x-1/2 w-full h-80 bg-gradient-to-r from-blue/10 via-indigo-500/5 to-purple-500/10 rounded-full blur-[90px] pointer-events-none -z-10" />

      {/* Tag pill — reduce size */}
      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-mat-surface border border-border/40 text-xs font-bold uppercase tracking-widest text-text-muted shadow-sm">
        <Sparkles className="w-3 h-3 text-brand-primary" />
        <span>Productivity Refined</span>
      </div>

      {/* H1 — reduce size on mobile */}
      <h1 className="text-2xl md:text-5xl font-black tracking-tighter leading-[1.1] text-text max-w-2xl mt-1">
      ...
      <p className="text-sm sm:text-base md:text-base text-text-muted font-medium max-w-sm sm:max-w-md mx-auto leading-snug sm:leading-relaxed">
        Local-first tools by KaruviLab.
        <span className="block text-text-muted mt-1">Fast. Secure. Local-first.</span>
      </p>
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-2 md:gap-x-4">
        {TRUST_ITEMS.map((item, i) => (
          <div key={i} className="flex items-center gap-1 md:gap-1.5 shrink-0">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-text-muted">{item.text}</span>
          </div>
        ))}
      </div>
