...
export default function EcdhClient() {
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      {/* Control Header */}
      <div className="p-5 rounded-2xl bg-surface-2 border border-border flex flex-wrap items-center justify-between gap-4 shadow-sm">
      ...
      {error && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm font-medium">
          {error}
        </motion.div>
      )}
          ...
          >
            ...
            <div className="space-y-4">
              <div className="p-5 rounded-2xl bg-surface-2 border border-border flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-base text-sky-400">Alice</h3>
              ...
              <div className="p-5 rounded-2xl bg-surface border border-border space-y-4 shadow-sm relative overflow-hidden">
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-semibold text-text-muted flex items-center gap-1.5">
                      <Key className="w-3.5 h-3.5 text-amber-400" /> Alice's Private Key
                    </label>
                  </div>
                  <textarea readOnly rows={3} value={partyAPrivate} className="w-full p-2.5 rounded-lg bg-surface-2 border border-border font-mono text-[11px] text-amber-300 resize-none focus:outline-none" />
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-semibold text-text-muted flex items-center gap-1.5">
                      <Lock className="w-3.5 h-3.5 text-sky-400" /> Alice's Public Key
                    </label>
                    <CopyButton text={partyAPublic} />
                  </div>
                  <textarea readOnly rows={3} value={partyAPublic} className="w-full p-2.5 rounded-lg bg-surface-2 border border-border font-mono text-[11px] text-sky-300 resize-none focus:outline-none" />
              ...
              <AnimatePresence>
                {step >= 2 && (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-indigo-300/70">Bob's Public Key</label>
                      <textarea readOnly rows={2} value={partyBPublic} className="w-full p-2.5 rounded-lg bg-surface/50 border border-indigo-500/20 font-mono text-[11px] text-indigo-300 resize-none focus:outline-none" />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
                ...
                {step >= 3 && (
                  ...
                  >
                    ...
                    <p className="text-xs text-emerald-300/80 leading-relaxed">
                      Calculated using <strong className="text-amber-300">Alice's Private Key</strong> + <strong className="text-indigo-300">Bob's Public Key</strong>
                    </p>
            ...
            <div className="space-y-4">
              <div className="p-5 rounded-2xl bg-surface-2 border border-border flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-base text-indigo-400">Bob</h3>
              ...
              <div className="p-5 rounded-2xl bg-surface border border-border space-y-4 shadow-sm relative overflow-hidden">
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-semibold text-text-muted flex items-center gap-1.5">
                      <Key className="w-3.5 h-3.5 text-amber-400" /> Bob's Private Key
                    </label>
                  </div>
                  <textarea readOnly rows={3} value={partyBPrivate} className="w-full p-2.5 rounded-lg bg-surface-2 border border-border font-mono text-[11px] text-amber-300 resize-none focus:outline-none" />
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-semibold text-text-muted flex items-center gap-1.5">
                      <Lock className="w-3.5 h-3.5 text-indigo-400" /> Bob's Public Key
                    </label>
                    <CopyButton text={partyBPublic} />
                  </div>
                  <textarea readOnly rows={3} value={partyBPublic} className="w-full p-2.5 rounded-lg bg-surface-2 border border-border font-mono text-[11px] text-indigo-300 resize-none focus:outline-none" />
                  <p className="text-[10px] text-text-muted">Safe to share over insecure channels.</p>
                </div>
              </div>
              ...
              <AnimatePresence>
                {step >= 2 && (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-sky-300/70">Alice's Public Key</label>
                      <textarea readOnly rows={2} value={partyAPublic} className="w-full p-2.5 rounded-lg bg-surface/50 border border-sky-500/20 font-mono text-[11px] text-sky-300 resize-none focus:outline-none" />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <AnimatePresence>
                {step >= 3 && (
                  ...
                  >
                    ...
                    <p className="text-xs text-emerald-300/80 leading-relaxed">
                      Calculated using <strong className="text-amber-300">Bob's Private Key</strong> + <strong className="text-sky-300">Alice's Public Key</strong>
                    </p>
                    <div className="p-3 rounded-xl bg-surface/50 border border-emerald-500/20 flex flex-col gap-2">
                      <div className="flex justify-end">
      ...
      <AnimatePresence>
        {matches && step === 3 && (
          <motion.div 
            initial={{ opacity: 0, y: 20 }} 
            animate={{ opacity: 1, y: 0 }}
            className="p-6 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 flex flex-col md:flex-row items-center gap-4 text-center md:text-left shadow-lg"
          >
