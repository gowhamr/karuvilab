(function() {
  ...
  function setupCombo(inputId: string, listId: string, onSelect: (city: City) => void) {
    const input = el(inputId) as HTMLInputElement;
    const list = el(listId) as HTMLDivElement;
    let filtered = cities;
    let highlight = 0;
    const render = () => {
    ...
    list.onmousedown = e => {
      ...
      const item = target.closest('.mch-combo-item') as HTMLElement;
  ...
  setupCombo('wc-src-input', 'wc-src-list', (c) => {
    wcSource = c;
    (el('wc-src-input') as HTMLInputElement).value = `${c.name}, ${c.country}`;
    calculate();
  });
  setupCombo('wc-add-input', 'wc-add-list', (c) => {
    if (!wcTargets.find(t => t.tz === c.tz && t.name === c.name)) wcTargets.push(c);
    (el('wc-add-input') as HTMLInputElement).value = '';
    calculate();
  });

  const today = new Date().toISOString().split('T')[0];
  const wcDateInput = el('wc-date') as HTMLInputElement;
  const wcSrcInput = el('wc-src-input') as HTMLInputElement;
  const wcTimeInput = el('wc-time') as HTMLInputElement;
  ...
  function calculate() {
    ...
    const wrap = el('wc-targets') as HTMLDivElement;
    if (!wrap) return;
    wrap.innerHTML = '';
  ...
  (window as any).removeTarget = (idx: number) => {
