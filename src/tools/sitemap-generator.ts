/* ===== sitemap-generator.ts – Generate XML sitemaps from URL lists ===== */

document.addEventListener('DOMContentLoaded', () => {
  const el = (id: string) => document.getElementById(id);
  const input = el('url-list') as HTMLTextAreaElement;
  const result = el('sitemap-result') as HTMLDivElement;
  const copyBtn = el('copy-btn') as HTMLButtonElement;
  const downloadBtn = el('download-btn') as HTMLButtonElement;

  const changeFreq = el('change-freq') as HTMLSelectElement;
  const priority = el('priority') as HTMLSelectElement;
  ...
  copyBtn.onclick = () => {
    ...
    navigator.clipboard.writeText(result.textContent || '').then(() => {
      (window as any).Shell.toast('XML copied!', 'success');
    });
  };
