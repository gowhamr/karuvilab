/* ===== currency-converter.ts – Live Currency Conversion logic ===== */
...
(function () {
  ...
  function mchWriteCache(c: any) { try { localStorage.setItem(MCH_CACHE_KEY, JSON.stringify(c)); } catch(e){} }
  
  function mchGetCached(base: string) {
  ...
  async function mchFetchRates(base: string) {
    try {
  ...
  function render(amt: number, rate: number, meta: any) {
    const res = el('cur-result');
    if (!res) return;
    const converted = amt * rate;
    const fromCur = (el('cur-from') as HTMLSelectElement).value;
    const toCur = (el('cur-to') as HTMLSelectElement).value;
    
    // @ts-ignore
    const esc = (s: string) => window.Utils ? window.Utils.escHtml(s) : s;

    res.innerHTML = `
      <div class="result-card">
        <div style="font-size:0.75rem; color:var(--text-3); text-transform:uppercase; font-weight:700;">Converted Amount</div>
        <div style="font-size:1.8rem; font-weight:800; color:var(--blue)">${converted.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})} ${esc(toCur)}</div>
  ...
  async function convert() {
    const amt = +(el('cur-amt') as HTMLInputElement).value;
    const from = (el('cur-from') as HTMLSelectElement).value;
    const to = (el('cur-to') as HTMLSelectElement).value;
    const res = el('cur-result');
    if (!res) return;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    try {
      const curFrom = el('cur-from') as HTMLSelectElement;
      const curTo = el('cur-to') as HTMLSelectElement;
