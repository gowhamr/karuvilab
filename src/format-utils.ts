...
const FormatUtils = (() => {
  ...
  function needsSpecialRead(file: File): boolean {
    return SPECIAL_READ.includes(Utils.getExt(file.name));
  }

  async function loadAny(file: File): Promise<HTMLImageElement | HTMLCanvasElement> {
    const ext = Utils.getExt(file.name);
  ...
  async function loadHeic(file: File): Promise<HTMLImageElement> {
  ...
  async function loadTiff(file: File): Promise<HTMLCanvasElement> {
  ...
  function createFileCard(file: File, onRemove?: () => void): HTMLElement {
    const ext   = Utils.getExt(file.name);
  ...
  function createResultCard(filename: string, blob: Blob, label: string = 'Converted'): HTMLElement {
    ...
    (card.querySelector('.dl-btn') as HTMLButtonElement).onclick = () => Utils.downloadBlob(blob, filename);
    return card;
  }

  return {
    FORMAT_INFO, ALL_IMAGE_EXTS,
    isImage, needsSpecialRead,
    loadAny, loadHeic, loadTiff,
    encodeTiff, encodeBmp,
    drawElement, colorFor,
    jsonToCsv,
    createFileCard, createResultCard,
  };
})();
