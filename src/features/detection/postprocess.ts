...
function calculateIoU(boxA: DetectedObjectBox, boxB: DetectedObjectBox): number {
...
export function applyNMS(candidates: DetectedObjectBox[], iouThreshold = 0.45): DetectedObjectBox[] {
  // Sort candidates descending by confidence
  const sorted = [...candidates].sort((a, b) => b.confidence - a.confidence);
  const selected: DetectedObjectBox[] = [];
...
export function processDetectionOutputs(
  ...
  const candidates: DetectedObjectBox[] = [];
