...
import { FinancialInputs, FinancialResults, Scenario } from './types';
...
export const useFinancialFreedomStore = create<FinancialFreedomState>()(
  persist(
    (set, get) => ({
      ...
      saveScenario: (name: string) => {
        set((state) => {
          if (state.scenarios.length >= 3) return state; // Max 3 scenarios
