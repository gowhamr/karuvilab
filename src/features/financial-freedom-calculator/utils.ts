import { FinancialInputs, FinancialResults, ProjectionData } from './types';
import { MAX_AGE } from './constants';

export function calculateFIRE(inputs: FinancialInputs): FinancialResults {
  const {
    currentAge,
    retirementAge,
    currentSavings,
  ...
  for (let age = currentAge; age <= MAX_AGE; age++) {
    ...
    if (!isRetired) {
      ...
      if (currentBalance < 0) currentBalance = 0; // Can't have negative balance
    }
