      ...
      {isOpen && (
        <>
          ...
          <m.div
            ...
            className="fixed bottom-0 left-0 right-0 bg-surface/80 backdrop-blur-2xl border-t border-border/30 rounded-t-6xl z-modal p-10 shadow-[0_-20px_80px_rgba(0,0,0,0.2)] max-h-[90vh] overflow-y-auto no-scrollbar md:hidden"
          >
            {/* Handle */}
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-4">
                ...
                {dayEvents.length === 0 ? (
                  <div className="p-10 border-2 border-dashed border-border/20 rounded-5xl text-center space-y-4">
                    <div className="w-16 h-16 bg-indigo-500/5 rounded-3xl flex items-center justify-center mx-auto text-3xl">📭</div>
                    <div className="space-y-1">
                      <p className="text-sm text-text-2 font-bold tracking-tight">No events scheduled</p>
                      <p className="text-xs text-text-4 font-medium">Enjoy your free time or add a new event.</p>
                    </div>
                    <button 
                      onClick={() => {
                        onAddEvent(date);
                      ...
                      className="mt-2 px-5 py-2.5 bg-surface border border-border/30 rounded-xl text-xs font-bold text-indigo-500 hover:bg-indigo-500/10 transition-colors"
                    >
                      Add Event
                    </button>
                  </div>
