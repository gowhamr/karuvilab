  ...
  return (
    <ToolWorkspace
      ...
      tabs={{
        ...
        activeId: mode,
        onChange: (id) => setMode(id as "match" | "replace")
      }}
      input={
        <div className="space-y-6 flex flex-col h-full">
          <div className="space-y-2">
            <label htmlFor="regex-pattern" className="text-sm font-bold text-text-2">Pattern</label>
            <div className="flex items-center bg-bg border border-border rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue transition-all h-12">
          ...
          <div className="space-y-2 flex-1 flex flex-col">
            ...
            <textarea
              ...
              className={`w-full flex-1 min-h-32 px-4 py-3 bg-bg border border-border rounded-xl font-mono focus:ring-2 focus:ring-blue outline-none transition-all resize-none ${wordWrap ? 'whitespace-pre-wrap' : 'whitespace-pre overflow-x-auto'}`}
      ...
      output={
        <div className="space-y-6 h-full flex flex-col">
          <h2 className="text-sm font-bold text-text-2">Results</h2>
          ...
          {!result?.error && !result?.matches?.length && !replaceOutput && (
            <div className="flex-1 flex items-center justify-center border border-dashed border-border rounded-xl p-6 text-text-4 italic text-sm">
              Results will appear here...
            </div>
          )}

          {mode === "replace" && replaceOutput !== null && !result?.error && (
            <div className="flex-1 space-y-2 flex flex-col">
              <pre className="w-full flex-1 min-h-32 px-4 py-3 bg-bg border border-border rounded-xl font-mono text-sm text-text whitespace-pre-wrap break-words overflow-auto custom-scrollbar">
                {replaceOutput}
              </pre>
            </div>
          )}

          {mode === "match" && result && !result.error && (
            <div className="space-y-4 flex-1 flex flex-col">
              <dl className="bg-bg border border-border p-4 rounded-xl flex items-center gap-4">
              ...
              {highlighted && (
                <div className="flex-1 flex flex-col space-y-2 min-h-32">
                  <h3 className="text-xs font-bold text-text-3 uppercase tracking-widest">Highlighted Matches</h3>
                  <div className="font-mono text-sm text-text break-all leading-relaxed whitespace-pre-wrap bg-bg border border-border rounded-xl p-4 flex-1 overflow-auto custom-scrollbar">
              ...
              {result.matches.length > 0 && (
                <div className="space-y-2">
                  <h3 className="text-xs font-bold text-text-3 uppercase tracking-widest">Match Details</h3>
