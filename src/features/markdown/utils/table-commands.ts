 ...
 */
export function isCursorInTable(editor: Editor | null): boolean {
 ...
 */
export function addRow(editor: Editor, position: 'below' | 'above' = 'below'): boolean {
  if (!editor || editor.isDestroyed) return false;
  ...
  if (!tableCell || !tableRow) return false;
 ...
 */
export function addColumn(editor: Editor, position: 'right' | 'left' = 'right'): boolean {
  if (!editor || editor.isDestroyed) return false;
  const { state, view } = editor;
  const { $from } = state.selection;
  ...
  if (tableDepth === -1 || rowDepth === -1 || cellDepth === -1) return false;

  const { tableCell, tableHeader } = state.schema.nodes;
  if (!tableCell || !tableHeader) return false;
...
export function deleteColumn(editor: Editor): boolean {
  ...
  let tr = state.tr;
  const rowPositions: { start: number; cellStart: number; cellEnd: number }[] = [];
  let offset = tablePos + 1;
  for (let r = 0; r < tableNode.childCount; r++) {
    const row = tableNode.child(r);
    let cellOffset = offset + 1;
