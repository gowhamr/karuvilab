...
export interface TokenizedMarkdown {
  /** Plain text for the spell-check engine — no Markdown syntax, code, URLs. */
  plainText: string;
  /**
   * Parallel array, same length as plainText.
   * offsetMap[i] = index into the original rawMd string for plainText[i].
   */
...
// Main export
// ---------------------------------------------------------------------------
export function tokenizeMarkdownForSpellCheck(rawMd: string): TokenizedMarkdown {
  if (!rawMd) return { plainText: "", offsetMap: new Int32Array(0) };
  ...
  const fencedSet = new Set<number>();
  FENCE_RE.lastIndex = 0;
  let fm: RegExpExecArray | null;
  while ((fm = FENCE_RE.exec(src)) !== null) {
    const abs = baseOffset + fm.index;
    for (let k = abs; k < abs + fm[0].length; k++) fencedSet.add(k);
  }
...
function processInline(
  ...
  outChars:     string[],
  outOffsets:   number[],
  fencedSet:    Set<number>,
): void {
  if (!line) return;

  // Build a sorted list of ranges to skip or replace
  interface Range { s: number; e: number; repl?: string; }
  const skip: Range[]    = [];   // emit space
  const replace: Range[] = [];   // emit repl string
  ...
  // Build a unified, sorted interval list. Skips take priority over replaces.
  interface Interval { s: number; e: number; kind: 'skip' | 'replace'; repl?: string; }
  const intervals: Interval[] = [
    ...skip.map(r    => ({ ...r, kind: 'skip'    as const })),
    ...replace.map(r => ({ ...r, kind: 'replace' as const })),
  ].sort((a, b) => a.s - b.s);
  ...
  while (i < len) {
    ...
    for (const iv of intervals) {
      ...
      if (iv.s === i) {
        if (iv.kind === 'skip') {
          ...
          if (repl.length > 0) pushSpace(outChars, outOffsets, lineRawStart + iv.e);
        }
        i = iv.e;
        continue outer;
      }
    ...
    if (fencedSet.has(lineRawStart + i)) { i++; continue; }
...
// Helpers
// ---------------------------------------------------------------------------
function pushSpace(chars: string[], offsets: number[], pos: number): void {
