...
export class MermaidExporter {
  ...
  public static async svgToDataUrl(svg: SVGSVGElement): Promise<string> {
    try {
      ...
      if (width > 0 && !clone.getAttribute('width')) clone.setAttribute('width', width.toString());
      if (height > 0 && !clone.getAttribute('height')) clone.setAttribute('height', height.toString());
