...
export class MermaidRenderManager {
  private static instance: MermaidRenderManager;
  private currentTheme: MermaidThemeMode | null = null;
  private isInitialized = false;
  private metrics: MermaidMetrics = {
    totalRenders: 0,
    cacheHits: 0,
    cacheMisses: 0,
    renderErrors: 0,
  ...
  public static getInstance(): MermaidRenderManager {
  ...
  public async renderDiagram(
    ...
    if (!preflight.valid) {
      ...
      const errorKind: MermaidErrorKind = preflight.complexity.complexity === 'blocked'
        ? 'COMPLEXITY_LIMIT'
        : 'SYNTAX_ERROR';
    ...
    const { promise } = mermaidQueue.enqueue(
      block.id,
   ...
   */
  public async waitForMermaidReady(timeoutMs: number = 8000): Promise<void> {
  ...
  public getMetrics(): MermaidMetrics {
    return {
      ...this.metrics,
