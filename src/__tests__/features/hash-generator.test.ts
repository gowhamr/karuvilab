...
describe('Hash Generator Native Tests', () => {
  ...
  it('verifies test vectors for new algorithms', async () => {
    ...
    for (const vector of testVectors) {
      let hasher: any;
      if (vector.algo === 'SHA3-256') hasher = await createSHA3(256);
      else if (vector.algo === 'SHA3-512') hasher = await createSHA3(512);
      else if (vector.algo === 'BLAKE3') hasher = await createBLAKE3();
