...
// Pure validation logic from src/validator.ts — no DOM or File API needed.
...
function checkFormat(ext: string, rule: DocRule): boolean {
  return rule.formats.includes(ext);
}

function checkSize(sizeKB: number, rule: DocRule): boolean {
  if (rule.minKB && sizeKB < rule.minKB) return false;
  if (rule.maxKB && sizeKB > rule.maxKB) return false;
  return true;
}
