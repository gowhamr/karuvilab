/* ===== validator.ts – file validation rules ===== */

// VALIDATE-001: All allowed formats explicitly typed and validated at runtime
const Validator = (() => {
  ...
  async function validate(file: File, docType: string = 'general'): Promise<ValidationResult> {
    ...
    checks.push({
      ...
      detail: nameOk
        ...
        : `Contains special characters. Safe name: "${Utils.safeName(file.name)}"`,
    });
    ...
    if (rule.minKB || rule.maxKB) {
      ...
      const rangeStr =
        rule.minKB && rule.maxKB ? `${rule.minKB}–${rule.maxKB} KB` :
      ...
      try {
        ...
        if (rule.exactW && rule.exactH) {
          ...
          checks.push({
            ...
            warn:   false,
            detail: dimOk
              ? `Correct: ${rule.exactW}×${rule.exactH} px`
              : `Required: ${rule.exactW}×${rule.exactH} px`,
          });
          ...
          checks.push({
            ...
            warn:   false,
            detail: wOk
              ? `Within max width ${rule.maxWidthPx} px`
              : `Exceeds max width ${rule.maxWidthPx} px`,
          });
        } else {
  ...
  function getRules(): Record<string, DocRule> {
    return DOC_RULES;
  }
