/* ===== image-tools.ts – compress, convert, create images ===== */
...
const ImageTools = (() => {
  ...
  async function resolveMime(fmtKey: string): Promise<ResolvedMime> {
  ...
  async function compress(file: File, opts: CompressionOptions = {}): Promise<CompressResult> {
  ...
  async function convert(file: File, targetFmt: string, quality: number = 0.85): Promise<ConvertResult> {
    const src    = await FormatUtils.loadAny(file);
  ...
  async function create(opts: CreateOptions): Promise<CreateResult> {
    const {
      width, height,
      bg = '#ffffff',
      format = 'jpeg',
    ...
    if (srcFile) {
      ...
      const sw0 = (src as HTMLImageElement).naturalWidth  ?? (src as HTMLCanvasElement).width;
      const sh0 = (src as HTMLImageElement).naturalHeight ?? (src as HTMLCanvasElement).height;
  ...
  async function autoFix(file: File, docType: string): Promise<Blob> {
