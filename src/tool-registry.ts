 ...
 *
 * Central catalogue of every tool on KaruviLab. Powers:
 *   • Homepage search and filtering
 *   • Recent + Popular tool sections
 *   • Category navigation
 *
 * Exposes globals on window.KaruviRegistry to avoid ES-module imports
 * (build emits non-bundled ESM that is loaded via plain <script>).
 */

(function () {
  type Category = 'calculators' | 'pdf' | 'image' | 'security' | 'developer' | 'utilities' | 'seo';
  ...
  const RECENT_PATH_KEY = 'karuvi.recent.paths';

  function findToolById(id: string): ToolEntry | undefined {
    return TOOLS.find(t => t.id === id);
  }

  function findToolByPath(pathname: string): ToolEntry | undefined {
  ...
  function getRecentTools(): ToolEntry[] {
    try {
  ...
  (window as Window & { KaruviRegistry?: unknown }).KaruviRegistry = {
    TOOLS,
    CATEGORIES,
    findToolById,
    findToolByPath,
