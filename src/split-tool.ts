/* ===== split-tool.ts – KaruviLab Split & Copy Tool ===== */

(function () {

  interface SplitToolElements {
    ...
    partsSummary:   HTMLElement;
    totalCharCount: HTMLElement;
    warningBanner:  HTMLElement | null;
    dropZone:       HTMLElement;
    fileInput:      HTMLInputElement;
    stickyActions:  HTMLElement;
    copyAllSeqBtn:  HTMLButtonElement;
    downloadBtn:    HTMLButtonElement;
    paramsContainer: HTMLElement | null;
    partsCount:     HTMLInputElement;
    maxChars:       HTMLInputElement;
    delimiterSelect: HTMLSelectElement;
    customDelim:    HTMLInputElement;
    customPartsCount: HTMLInputElement;
    partsMinus:     HTMLButtonElement | null;
    partsPlus:      HTMLButtonElement | null;
  }
  ...
  const SplitTool = {
    state: {
      parts: [] as string[],
      content: '',
      currentMethod: 'equal' as SplitMethod,
    },

    els: null as SplitToolElements | null,
    ...
    cacheElements(): void {
      ...
      this.els = {
        input:            g('split-input')            as HTMLTextAreaElement,
        methodSelect:     g('split-method')           as HTMLSelectElement,
        splitBtn:         g('split-btn')              as HTMLButtonElement,
        clearBtn:         g('clear-btn')              as HTMLButtonElement,
        resultsSection:   g('results-section')        as HTMLElement,
        resultsArea:      g('split-results')          as HTMLElement,
        partsSummary:     g('parts-summary')          as HTMLElement,
        totalCharCount:   g('total-char-count')       as HTMLElement,
        warningBanner:    g('large-content-warning'),
        dropZone:         g('drop-zone')              as HTMLElement,
        fileInput:        g('file-input')             as HTMLInputElement,
        stickyActions:    g('sticky-actions')         as HTMLElement,
        copyAllSeqBtn:    g('copy-all-seq-btn')       as HTMLButtonElement,
        downloadBtn:      g('download-btn')           as HTMLButtonElement,
        paramsContainer:  g('method-params-container'),
        partsCount:       g('parts-count')            as HTMLInputElement,
        maxChars:         g('max-chars')              as HTMLInputElement,
        delimiterSelect:  g('split-delimiter')        as HTMLSelectElement,
        customDelim:      g('custom-delim')           as HTMLInputElement,
        customPartsCount: g('custom-parts-count')     as HTMLInputElement,
        partsMinus:       g('parts-minus')            as HTMLButtonElement | null,
        partsPlus:        g('parts-plus')             as HTMLButtonElement | null,
      };
    },
    ...
    bindEvents(): void {
      ...
      els.methodSelect.addEventListener('change', (e: Event) => {
        this.state.currentMethod = (e.target as HTMLSelectElement).value as SplitMethod;
        this.updateMethodUI();
        this.executeSplit();
      });
      ...
      els.fileInput.addEventListener('change', (e: Event) => this.handleFileSelect(e));

      els.dropZone.addEventListener('dragover', (e: Event) => {
        e.preventDefault();
      ...
      if (els.partsMinus) {
        els.partsMinus.addEventListener('click', () => {
          ...
          if (v > 2) { els.partsCount.value = String(v - 1); this.executeSplit(); }
        });
      }
      if (els.partsPlus) {
        els.partsPlus.addEventListener('click', () => {
          const v = parseInt(els.partsCount.value, 10);
          if (v < 50) { els.partsCount.value = String(v + 1); this.executeSplit(); }
        });
      }
    },
    ...
    handleInputChange(): void {
      ...
      if (els.warningBanner) els.warningBanner.style.display = len > 50000 ? 'block' : 'none';
    },
    ...
    updateMethodUI(): void {
      ...
      els.paramsContainer.querySelectorAll('[data-method]').forEach(el => {
        (el as HTMLElement).style.display = (el as HTMLElement).dataset.method === method ? '' : 'none';
      });
    },
    ...
    handleFiles(files: FileList): void {
      ...
      reader.onload  = (e: ProgressEvent<FileReader>) => {
        const { els } = this;
        if (!els) return;
        els.input.value = e.target?.result as string ?? '';
        this.handleInputChange();
        this.executeSplit();
      };
    ...
    executeSplit(): void {
      ...
      if (method === 'equal') {
        ...
        const max = parseInt(els.maxChars.value, 10) || 1000;
    ...
    splitEqually(text: string, count: number): string[] {
      const parts: string[] = [];
    ...
    splitByChars(text: string, max: number): string[] {
      const parts: string[] = [];
    ...
    splitByDelimiter(text: string): string[] {
      const { els } = this;
      if (!els) return [text];
      const dType = els.delimiterSelect.value;
      let delim: RegExp | string = '\n';
    ...
    renderResults(): void {
      ...
      els.resultsArea.querySelectorAll('.copy-btn').forEach(btn => {
        btn.addEventListener('click', (e: Event) => {
          ...
          this.copyPart(idx, e.target as HTMLButtonElement);
        });
      });
    },

    async copyPart(index: number, btn: HTMLButtonElement): Promise<void> {
    ...
    async copySequential(): Promise<void> {
      const { parts } = this.state;
      const { els }   = this;
      if (!els) return;
