...
export type SampleAssetKey = keyof typeof SAMPLE_ASSETS;

/**
 * Utility to load a sample asset based on tool ID.
 * This function returns the raw asset which the tool then processes.
 */
export async function loadSample(toolId: string): Promise<any> {
  const asset = SAMPLE_ASSETS[toolId as SampleAssetKey];
