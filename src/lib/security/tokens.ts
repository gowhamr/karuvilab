...
export function parseJwt(token: string): JwtParsed {
  ...
  let issuedAtDate: string | undefined = undefined;
  ...
  try {
    ...
    if (alg.startsWith("HS")) {
      ...
      const key = await crypto.subtle.importKey(
        "raw",
        new TextEncoder().encode(secretOrPublicKeyPem),
    ...
    if (alg.startsWith("RS") || alg.startsWith("PS")) {
      ...
      const key = await crypto.subtle.importKey("spki", keyBytes.buffer as ArrayBuffer, cryptoAlgo, false, ["verify"]);
      const valid = await crypto.subtle.verify(
        alg.startsWith("RS") ? "RSASSA-PKCS1-v1_5" : { name: "RSA-PSS", saltLength: 32 },
        key,
...
export function parseSaml(input: string): SamlParsed {
  ...
  const isRequest = Boolean(
    doc.querySelector("AuthnRequest, samlp\\:AuthnRequest, LogoutRequest, samlp\\:LogoutRequest")
  );
