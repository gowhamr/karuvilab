...
export const ISO8583_RESPONSE_CODES: Record<string, string> = {
...
export interface ParsedISOField {
  ...
  decodedInfo?: string | undefined;
}

export interface MTIDecoded {
  version: string;
  class: string;
  function: string;
  originator: string;
...
export function decodeMTI(mti: string): MTIDecoded {
  if (mti.length !== 4) {
...
export function parseIso8583(msgInput: string) {
  ...
  for (const fieldNum of presentFields) {
    ...
    if (def.type === 'FIXED') {
      ...
      if (offset + len > payload.length) throw new Error(`Unexpected end of data at Field ${fieldNum} content (expected ${len} chars, remaining: ${payload.length - offset})`);
    ...
    let decodedInfo: string | undefined = undefined;
    if (fieldNum === 3) {
