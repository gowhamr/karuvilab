const Validator = /* @__PURE__ */ (() => {
  ...
  async function validate(file, docType = "general") {
    ...
    checks.push({
      ...
      pass: fmtOk,
      warn: false,
      detail: fmtOk ? `Allowed formats: ${rule.formats.join(", ").toUpperCase()}` : `Not allowed. Use: ${rule.formats.join(", ").toUpperCase()}`
    });
    ...
    if (rule.minKB || rule.maxKB) {
      ...
      const rangeStr = rule.minKB && rule.maxKB ? `${rule.minKB}\u2013${rule.maxKB} KB` : rule.maxKB ? `\u2264${rule.maxKB} KB` : `\u2265${rule.minKB} KB`;
      checks.push({
    ...
    if (ext !== "pdf") {
      try {
        ...
        if (rule.exactW && rule.exactH) {
          ...
          checks.push({
            label: `Dimensions: ${w}\xD7${h} px`,
            pass: dimOk,
            warn: false,
            detail: dimOk ? `Correct: ${rule.exactW}\xD7${rule.exactH} px` : `Required: ${rule.exactW}\xD7${rule.exactH} px`
          });
          ...
          checks.push({ label: `Dimensions: ${w}\xD7${h} px`, pass: true, warn: false, detail: "No dimension restriction" });
        }
      } catch {
