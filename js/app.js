document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".doc-type-chip").forEach((chip) => {
    chip.addEventListener("click", () => {
      ...
      if (sel) {
        sel.value = chip.dataset.docType;
  ...
  function closeFaq() {
    ...
    setDockActive(
      activePanel === "history" ? "history" : TOOL_PANELS.includes(activePanel) ? "tools" : "home"
    );
  }
  ...
  function setSliderVal(id, valId, val, fmt) {
    ...
    if (s) {
      s.value = String(val);
      s.setAttribute("aria-valuenow", String(val));
    }
  ...
  async function runCompressor() {
    ...
    if (compressBtn) {
      ...
      compressBtn.innerHTML = Utils.spinnerHTML() + " Processing\u2026";
    }
    ...
    const maxWidth = Number(document.getElementById("img-max-width")?.value) || 1e3;
    const resultBlobs = [];
    let html = "";
    let successCount = 0;
    try {
      for (const file of compressorFiles) {
        ...
        try {
          ...
          window.Shell.toast(`Error processing ${file.name}: ${err.message}`, "error");
        }
      }
    } finally {
  ...
  convertBtn?.addEventListener("click", async () => {
    ...
    if (convertBtn) {
      ...
      convertBtn.innerHTML = Utils.spinnerHTML() + " Converting\u2026";
    }
    ...
    try {
      for (const file of converterFiles) {
        ...
        try {
          ...
          html += errorCard(file.name, err.message);
          resultBlobs.push(null);
          window.Shell.toast(`Error converting ${file.name}: ${err.message}`, "error");
        }
      }
    } finally {
  ...
  document.querySelectorAll(".preset-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      document.querySelectorAll(".preset-btn").forEach((b2) => b2.classList.remove("selected"));
  ...
  ["dragenter", "dragover"].forEach(
    (ev) => creatorZone?.addEventListener(ev, (e) => {
      e.preventDefault();
      creatorZone.classList.add("drag-over");
    })
  );
  ["dragleave", "drop"].forEach(
    (ev) => creatorZone?.addEventListener(ev, (e) => {
      e.preventDefault();
      creatorZone.classList.remove("drag-over");
    })
  );
  creatorZone?.addEventListener("drop", (e) => {
  ...
  function setCreatorFile(f) {
    ...
    if (p) p.innerHTML = `Loaded: <strong>${Utils.escHtml(f.name)}</strong> \xB7 <label class="link" for="creator-input">change</label>`;
  }
  document.getElementById("create-btn")?.addEventListener("click", async () => {
    ...
    try {
      const bg = document.getElementById("create-bg")?.value ?? "#ffffff";
      const fmt = document.getElementById("create-format")?.value ?? "jpeg";
      const { blob, canvas: c, fmtKey, fallback } = await ImageTools.create({
        width: w,
        height: h,
        bg,
        format: fmt,
        srcFile: creatorSrcFile,
      ...
      if (resultEl) resultEl.innerHTML = errorCard("Create", err.message);
      window.Shell.toast("Creation failed: " + err.message, "error");
    }
  });
  ...
  ["dragenter", "dragover"].forEach(
    (ev) => validatorDrop?.addEventListener(ev, (e) => {
      e.preventDefault();
      validatorDrop.classList.add("drag-over");
    })
  );
  ["dragleave", "drop"].forEach(
    (ev) => validatorDrop?.addEventListener(ev, (e) => {
      e.preventDefault();
      validatorDrop.classList.remove("drag-over");
    })
  );
  validatorDrop?.addEventListener("drop", (e) => {
    const f = e.dataTransfer?.files[0];
    if (f) runValidator(f);
  });
  ...
  async function runValidator(file) {
    ...
    const docType = document.getElementById("validator-doc-type")?.value ?? "general";
        ...
        if (autoFixBtn) {
          autoFixBtn.addEventListener("click", async () => {
            ...
            try {
              ...
              autoFixBtn.innerHTML = "&#10060; " + Utils.escHtml(e.message);
              window.Shell.toast("Auto-fix failed: " + e.message, "error");
            }
          });
        }
      }
    } catch (err) {
      if (resultEl) resultEl.innerHTML = errorCard(file.name, err.message);
      window.Shell.toast("Validation error: " + err.message, "error");
    }
  }
  function buildResultCard(origFile, blob, verb, kind, targetFmt, extraNote = "") {
    ...
    const newName = verb === "converted" ? Utils.replaceExt(origFile.name, extOut) : (() => {
  ...
  (function initRegex() {
    ...
    document.querySelectorAll(".regex-preset-chip").forEach((chip) => {
      chip.addEventListener("click", () => {
        document.querySelectorAll(".regex-preset-chip").forEach((c2) => c2.classList.remove("active"));
        chip.classList.add("active");
        const c = chip;
        patternInput.value = c.dataset.pattern ?? "";
        const flags = c.dataset.flags ?? "";
        const gf = document.getElementById("rf-g");
        if (gf) gf.checked = flags.includes("g");
        const ifl = document.getElementById("rf-i");
        if (ifl) ifl.checked = flags.includes("i");
        const mf = document.getElementById("rf-m");
        if (mf) mf.checked = flags.includes("m");
      });
    });
    function getFlags() {
    ...
    function applyRegexString(raw) {
      ...
      if (m && patternInput) {
        ...
        const gf = document.getElementById("rf-g");
        if (gf) gf.checked = m[2].includes("g");
        const ifl = document.getElementById("rf-i");
        if (ifl) ifl.checked = m[2].includes("i");
        const mf = document.getElementById("rf-m");
        if (mf) mf.checked = m[2].includes("m");
        return true;
      }
      return false;
    }
    ...
    runBtn.addEventListener("click", () => {
      ...
      regexWorker.onmessage = function(e) {
        ...
        if (matchCount) {
          matchCount.textContent = matches.length ? `${matches.length} match${matches.length > 1 ? "es" : ""} found` : "No matches found";
