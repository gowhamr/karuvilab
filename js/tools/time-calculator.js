(function() {
  ...
  let timeMode = "add";
  const btns = document.querySelectorAll("#time-toggle button");
  btns.forEach((b) => {
    b.onclick = () => {
  ...
  if (calcBtn) {
    calcBtn.onclick = () => {
      const h1 = +el("t1-h").value || 0;
      const m1 = +el("t1-m").value || 0;
      const s1 = +el("t1-s").value || 0;
      const h2 = +el("t2-h").value || 0;
      const m2 = +el("t2-m").value || 0;
      const s2 = +el("t2-s").value || 0;
