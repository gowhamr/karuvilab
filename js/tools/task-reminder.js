document.addEventListener("DOMContentLoaded", () => {
  ...
  const input = el("task-in");
  const addBtn = el("add-task-btn");
  const container = el("task-container");
  const footer = el("task-footer");
  const stats = el("task-stats");
  const clearBtn = el("clear-completed");
  let tasks = JSON.parse(localStorage.getItem("karuvi_tasks") || "[]");
  function save() {
  ...
  function render() {
    ...
    tasks.forEach((task) => {
      ...
      item.innerHTML = `
        ...
        <span class="remove-task">\xD7</span>
      `;
