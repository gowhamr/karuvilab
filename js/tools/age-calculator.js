(function() {
  ...
  function calculate() {
    const dobVal = el("age-dob").value;
    const asofVal = el("age-as").value;
    if (!dobVal || !asofVal) {
    ...
    const totalDays = Math.floor((asof.getTime() - dob.getTime()) / (1e3 * 60 * 60 * 24));
  ...
  document.addEventListener("DOMContentLoaded", () => {
    try {
      ...
      const asOfInput = el("age-as");
      const dobInput = el("age-dob");
