...
### 2026-08-03 — Backfilled Missing Entries (Audit Remediation)

- **Library:** `@monaco-editor/react` & `monaco-editor`
- **Impact:** ~2.5MB gzipped (Dynamic Import strictly required)
- **Purpose:** Full-featured code editor with syntax highlighting, minimap, and intellisense for developer tools.
- **Alternatives:** `CodeMirror` (lighter, but less out-of-the-box IDE features).
- **Justification:** Necessary for complex dev tools (JSON formatter, Code playground). Loaded asynchronously to avoid impacting initial bundle size.

- **Library:** `marked`
- **Impact:** ~10KB gzipped
- **Purpose:** Fast, lightweight markdown parsing.
- **Alternatives:** `markdown-it` (slightly heavier), `remark` (heavy AST ecosystem).
- **Justification:** Fast and standard markdown compiler to render markdown preview and documentation pages safely (paired with DOMPurify).

- **Library:** `highlight.js`
- **Impact:** ~15KB gzipped (core), language packs loaded dynamically
- **Purpose:** Code syntax highlighting in markdown previews and code viewers.
- **Alternatives:** `Prism.js` (comparable, but `highlight.js` integrates well with `marked`).
- **Justification:** Lightweight and reliable syntax highlighting.

- **Library:** `yaml`
- **Impact:** ~15KB gzipped
- **Purpose:** Parsing and stringifying YAML for format converters.
- **Alternatives:** `js-yaml` (slightly heavier).
- **Justification:** Compliant and modern YAML parser for developer utilities.

- **Library:** `date-fns-tz`
- **Impact:** ~5KB gzipped
- **Purpose:** Timezone support extension for `date-fns`.
- **Alternatives:** `moment-timezone` (deprecated, massive bundle size), `dayjs-tz` (requires changing core date library).
- **Justification:** Essential for time-conversion tools, extending the already-approved `date-fns` without adding a new base date library.

- **Library:** `@radix-ui/react-*` components
- **Impact:** ~1-5KB gzipped per component
- **Purpose:** Unstyled, accessible UI primitives for dropdowns, modals, sliders, and form controls.
- **Alternatives:** Custom implementations (high risk of accessibility/ARIA violations), Headless UI (comparable).
- **Justification:** Provides WCAG 2.2 AA compliant keyboard navigation, focus trapping, and screen reader support out of the box, fulfilling accessibility standards.

- **Library:** `lucide-react`
- **Impact:** < 1KB gzipped per icon (Tree-shaken)
- **Purpose:** Clean, consistent SVG icon set.
- **Alternatives:** Heroicons, Phosphor Icons.
- **Justification:** Industry-standard icons with excellent tree-shaking support, ensuring only used icons enter the bundle.

- **Library:** `sonner`
- **Impact:** ~4KB gzipped
- **Purpose:** Accessible toast notification system.
- **Alternatives:** `react-hot-toast` (similar size), custom implementation.
- **Justification:** Highly performant, accessible toast manager for user feedback and error surfacing (KL-08).

- **Library:** `clsx` & `tailwind-merge`
- **Impact:** ~2KB gzipped combined
- **Purpose:** Dynamic Tailwind class merging without style conflicts.
- **Alternatives:** Manual string concatenation (leads to CSS specificity bugs).
- **Justification:** Critical for building robust design system UI components that accept conditional classes.

- **Library:** `idb`
- **Impact:** ~1KB gzipped
- **Purpose:** Lightweight promise wrapper for IndexedDB native API.
- **Alternatives:** `localforage`, `dexie` (heavier wrappers).
- **Justification:** Core persistence layer for Zustand stores; minimal footprint over native IDB.

- **Library:** `docx`
- **Impact:** ~55KB gzipped (Dynamic Import)
- **Purpose:** Client-side generation of Microsoft Word (.docx) files.
- **Alternatives:** Server-side generation (violates zero-server-upload), custom XML builders (too complex/brittle).
- **Justification:** Necessary for exporting documents to Word format locally in the browser.

- **Library:** `pdfjs-dist`
- **Impact:** ~350KB gzipped (Dynamic Import in Web Worker strictly required)
- **Purpose:** Robust PDF rendering, page extraction, and text parsing.
- **Alternatives:** Native browser PDF viewers (no API access), `pdf-lib` (only for manipulation, poor text extraction/rendering).
- **Justification:** Essential for PDF-to-Image, PDF-to-Text, and PDF preview tools. Enforces PERF-04 via worker-only loading.
