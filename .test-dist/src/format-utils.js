...
export function needsSpecialRead(file) {
    return SPECIAL_READ.includes(Utils.getExt(file.name));
}
export async function loadAny(file) {
    const ext = Utils.getExt(file.name);
...
export async function loadHeic(file) {
...
export function drawElement(el, maxW, maxH) {
...
export async function extractGifFrames(file) {
    ...
    const decoder = new window.ImageDecoder({ type: 'image/gif', data: file.stream() });
