...
function getDefaultMaxCacheSize() {
    if (typeof navigator === 'undefined')
        return 500 * 1024 * 1024; // SSR fallback
 ...
 */
export async function evictLruModelCache(maxStorageBytes) {
...
export async function getCachedModel(modelId, version) {
    try {
        const db = await getDb();
        const record = (await db.get(STORE_NAME, modelId));
...
export async function saveCachedModel(modelId, version, buffer, maxStorageBytes) {
    try {
        const limit = maxStorageBytes || getDefaultMaxCacheSize();
...
export async function getAllCachedModelIds() {
    try {
        const db = await getDb();
