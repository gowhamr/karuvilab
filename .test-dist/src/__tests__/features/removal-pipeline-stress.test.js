...
import { runRemovalEngineBenchmark, BENCHMARK_TEST_MATRIX, calculateEdgeQualityScore } from '../../features/background-remover/benchmarks/benchmark-runner.js';
describe('Phase 8 & 9 — Benchmark, Stress & Memory Safety Suite', () => {
    it('should calculate accurate edge quality scores across different matte densities', () => {
        ...
        for (let i = 0; i < 100; i++) {
            if (i < 45)
                ...
                softMask[i] = (i - 45) / 10; // 10% smooth transition
            else
                softMask[i] = 1;
        }
    ...
    it('should abort immediately and safely on AbortSignal without lingering references', async () => {
        ...
        let threw = false;
        try {
            await canvasEngine.generateMask({ file, width: 200, height: 200 }, undefined, controller.signal);
        }
        catch {
            threw = true;
        }
