export const base64 = {
    detailedDescription: "Encode plain text or binary data to Base64 and decode Base64 strings back to readable text, all inside your browser. Supports both standard Base64 and the URL-safe variant (replacing `+` with `-` and `/` with `_`). Useful for embedding data in JSON, URLs, and HTTP headers without special-character conflicts.",
    howTo: [
    ...
    faq: [
        {
            ...
            answer: "No. Base64 is an encoding scheme, not encryption. Anyone who sees the Base64 string can trivially decode it.",
        },
        {
            question: "Why does my decoded output look garbled?",
            answer: "The input may be binary data (like an image) rather than text. Base64 can encode arbitrary bytes, but the decoded output will only be readable if the original was text.",
        },
        {
            question: "What is the URL-safe variant?",
            answer: "URL-safe Base64 replaces `+` with `-` and `/` with `_` so the output can be used directly in URLs and filenames without percent-encoding.",
        },
        {
            question: "Is there a size limit?",
            answer: "The tool handles typical text and small files. Very large inputs may slow the browser but there is no hard limit enforced.",
        },
    ],
    useCases: [
