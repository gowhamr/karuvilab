export const urlEncoder = {
    detailedDescription: "Percent-encode and decode URLs and query string parameters to ensure they are safely transmitted in HTTP requests. Special characters like spaces, `&`, `=`, and non-ASCII letters are replaced with their `%XX` hex equivalents. Everything runs in the browser — no data is sent anywhere.",
    howTo: [
    ...
    faq: [
        {
            ...
            answer: "Use `encodeURI` for full URLs (preserves `://`, `?`, `&`, `=`) and `encodeURIComponent` for individual query parameter values (encodes everything including `&` and `=`).",
        },
        {
            question: "Why does a space become `%20` or `+`?",
            answer: "Both are valid in different contexts. `%20` is the RFC-standard encoding; `+` is used only in `application/x-www-form-urlencoded` form data.",
        },
        {
            question: "Does this handle Unicode characters?",
            answer: "Yes. Non-ASCII characters are UTF-8-encoded first and then percent-encoded (e.g., `€` becomes `%E2%82%AC`).",
        },
    ],
    useCases: [
