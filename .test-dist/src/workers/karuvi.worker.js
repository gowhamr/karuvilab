...
async function sha(algo, input) {
    ...
    return await cryptoProvider.subtle.digest(algo.replace("SHA-", "SHA-"), bytes.buffer);
}
async function hmac(algo, key, input) {
    ...
    const cryptoKey = await cryptoProvider.subtle.importKey("raw", keyData, { name: "HMAC", hash: { name: algo.replace("SHA-", "SHA") } }, false, ["sign"]);
    return await cryptoProvider.subtle.sign("HMAC", cryptoKey, data);
}
const api = {
    // Security Worker Methods Stubs (handled by crypto.worker.ts)
    directoryHashManifest: async (files, algo, encoding = 'hex', onProgress) => {
        const manifest = [];
    ...
    // Hash Tasks
    async parseMarkdown(text, onProgress) {
        if (onProgress)
            onProgress({ percent: 10, message: "Loading parser..." });
        ...
        if (onProgress)
            onProgress({ percent: 50, message: "Parsing markdown..." });
    ...
    async generateHashes(text, algos, encoding = 'hex', onProgress) {
        ...
        if (algos.includes("MD5")) {
            ...
            current++;
            if (onProgress)
                onProgress({ percent: (current / total) * 100, message: "Computed MD5" });
        }
    ...
    async exportPdfEditor(file, pagesState, annotations, onProgress) {
        ...
        if (onProgress)
            onProgress({ percent: 10, message: "Loading original PDF..." });
        const srcDoc = await PDFDocument.load(file);
        const newDoc = await PDFDocument.create();
        if (onProgress)
            onProgress({ percent: 30, message: "Structuring pages..." });
        ...
        if (onProgress)
            onProgress({ percent: 60, message: "Applying annotations..." });
        const hexToRgb = (hex) => {
        ...
        for (const ann of annotations) {
            ...
            const getOriginalCoords = (vx, vy) => {
                if (pageRotation === 0)
                    return { x: vx, y: origH - vy };
                if (pageRotation === 90)
                    return { x: origW - vy, y: vx };
                if (pageRotation === 180)
                    return { x: origW - vx, y: vy };
                if (pageRotation === 270)
                    return { x: vy, y: origH - vx };
                return { x: vx, y: origH - vy };
            };
            if (ann.type === 'blackout') {
                        ...
                        if (stream instanceof PDFRawStream && ref instanceof PDFRef) {
                            try {
                                    ...
                                    if (match.endsWith('Tj') || match.endsWith('TJ')) {
                                        ...
                                        if (currentX >= blX - 20 && currentX <= trX + 20 &&
                                            ...
                                            if (match.endsWith('Tj'))
                                                return match.trim().startsWith('<') ? '<> Tj' : '() Tj';
                                            if (match.endsWith('TJ'))
                                                return '[] TJ';
                                        }
                                    }
                                ...
                                for (let j = 0; j < textStr.length; j++)
                                    encoded[j] = textStr.charCodeAt(j);
                ...
                for (const rect of ann.rects) {
                    ...
                    if (BlendMode)
                        opts.blendMode = BlendMode.Multiply;
                ...
                if (!img) {
                    ...
                    if (ann.dataUrl.includes('image/png'))
                        img = await newDoc.embedPng(buf);
                    else
                        img = await newDoc.embedJpg(buf);
        ...
        if (onProgress)
            onProgress({ percent: 90, message: "Saving flattened PDF..." });
    ...
    async rotatePdf(file, rotateAll, allAngle, pageAngles, onProgress) {
    ...
    async convertImagesToPdf(images, pageSize, onProgress) {
        const { PDFDocument } = await import("pdf-lib");
        const pdf = await PDFDocument.create();
        const PAGE_SIZES = { a4: [595.28, 841.89], letter: [612, 792] };
        for (let i = 0; i < images.length; i++) {
            if (onProgress)
                onProgress({ percent: (i / images.length) * 100, message: `Processing image ${i + 1}/${images.length}...` });
            const item = images[i];
            if (!item)
                continue;
            let img;
            if (item.mime === "image/png")
                img = await pdf.embedPng(item.buffer);
            else
                img = await pdf.embedJpg(item.buffer);
        ...
        if (onProgress)
            onProgress({ percent: 100, message: "Saving PDF..." });
        const bytes = await pdf.save();
        return Comlink.transfer(bytes, [bytes.buffer]);
    },
    async lockPdf(file, userPassword, ownerPassword, onProgress) {
        const { PDFDocument } = await import("pdf-lib");
        if (onProgress)
            onProgress({ percent: 50, message: "Loading PDF..." });
        const doc = await PDFDocument.load(file);
        if (onProgress)
            onProgress({ percent: 90, message: "Locking and saving PDF..." });
        const outBytes = await doc.save({
            userPassword,
    ...
    async unlockPdf(file, password, onProgress) {
        const { PDFDocument } = await import("pdf-lib");
        if (onProgress)
            onProgress({ percent: 50, message: "Unlocking PDF..." });
    ...
    async addPageNumbersToPdf(file, options, onProgress) {
        const { PDFDocument, rgb } = await import("pdf-lib");
        if (onProgress)
            onProgress({ percent: 10, message: "Loading PDF..." });
        const doc = await PDFDocument.load(file);
        const pages = doc.getPages();
        const hexToRgb = (hex) => ({
        ...
        for (let i = 0; i < pages.length; i++) {
            if (onProgress && i % 10 === 0)
                onProgress({ percent: 10 + (i / pages.length) * 80, message: `Adding page number ${i + 1}/${pages.length}...` });
            const page = pages[i];
            if (!page)
                continue;
            const { width, height } = page.getSize();
    ...
    async adjustPdfLayout(file, options, onProgress) {
        const { PDFDocument } = await import("pdf-lib");
        if (onProgress)
            onProgress({ percent: 10, message: "Loading PDF..." });
        const doc = await PDFDocument.load(file);
        ...
        if (options.action === 'crop' && options.cropBox) {
            for (let i = 0; i < targetPages.length; i++) {
                ...
                if (onProgress && i % 5 === 0)
                    onProgress({ percent: 10 + (i / targetPages.length) * 80, message: `Cropping page ${i + 1}/${targetPages.length}...` });
            ...
            if (onProgress)
                onProgress({ percent: 20, message: "Rebuilding pages..." });
            const newDoc = await PDFDocument.create();
            for (let i = 0; i < pageCount; i++) {
                if (onProgress && i % 5 === 0)
                    onProgress({ percent: 20 + (i / pageCount) * 70, message: `Processing page ${i + 1}/${pageCount}...` });
    ...
    async setPdfMetadata(file, metadata, onProgress) {
        const { PDFDocument } = await import("pdf-lib");
        if (onProgress)
            onProgress({ percent: 10, message: "Loading PDF..." });
        const doc = await PDFDocument.load(file);
        if (onProgress)
            onProgress({ percent: 50, message: "Updating metadata..." });
    ...
    async getPdfBookmarks(file, onProgress) {
        if (onProgress)
            onProgress({ percent: 20, message: "Loading PDF..." });
        ...
        if (onProgress)
            onProgress({ percent: 70, message: "Extracting bookmarks..." });
        const outline = await pdf.getOutline();
        // Convert outline into a simple array of nodes
        const serializeOutline = (nodes) => {
            if (!nodes)
                return [];
    ...
    async extractPdfAttachments(file, onProgress) {
        if (onProgress)
            onProgress({ percent: 20, message: "Loading PDF..." });
        const pdfjsLib = await import("pdfjs-dist");
        const version = pdfjsLib.version || '6.2.108';
        ...
        const pdf = await pdfjsLib.getDocument({ data: file }).promise;
        if (onProgress)
            onProgress({ percent: 70, message: "Extracting attachments..." });
    ...
    async mergePdfs(files, onProgress) {
        ...
        for (let i = 0; i < total; i++) {
            const file = files[i];
            if (onProgress)
                onProgress({ percent: (i / total) * 80, message: `Merging PDF ${i + 1}/${total}` });
        ...
        if (onProgress)
            onProgress({ percent: 90, message: "Saving merged PDF..." });
        const result = await merged.save();
        if (onProgress)
            onProgress({ percent: 100, message: "Done!" });
        const _arr = new Uint8Array(result);
        return Comlink.transfer(_arr, [_arr.buffer]);
    },
    async compressPdf(file, level = 'medium', onProgress) {
        if (onProgress)
            onProgress({ percent: 10, message: "Loading PDF..." });
        const { PDFDocument } = await import("pdf-lib");
        const loadOptions = {};
        ...
        if (onProgress)
            onProgress({ percent: 30, message: "Analyzing structure..." });
        ...
        if (onProgress)
            onProgress({ percent: 50, message: `Optimizing (${level} compression)...` });
        ...
        if (level === 'high') {
            if (onProgress)
                onProgress({ percent: 60, message: "Deep optimization — removing orphaned objects..." });
            ...
            doc2.setProducer('');
            if (onProgress)
                onProgress({ percent: 80, message: "Saving optimized PDF..." });
    ...
    async splitPdf(file, splitAll, rangesStr, onProgress) {
        if (onProgress)
            onProgress({ percent: 10, message: "Loading PDF..." });
        const { PDFDocument } = await import("pdf-lib");
        const srcDoc = await PDFDocument.load(file);
        ...
        for (let g = 0; g < groups.length; g++) {
            if (onProgress)
                onProgress({ percent: 10 + (g / groups.length) * 80, message: `Processing part ${g + 1}/${groups.length}...` });
        ...
        if (onProgress)
            onProgress({ percent: 95, message: "Zipping parts..." });
    ...
    async compressImage(file, mimeType, format, quality, onProgress) {
        let imgBitmap = null;
        try {
            ...
            if (!compressedBlob)
                throw new Error("Canvas export failed: Blob is null");
    ...
    async compressImageBatch(file, mimeType, settings, onProgress) {
        let imgBitmap = null;
        try {
            if (onProgress)
                onProgress({ percent: 10, message: "Decoding image..." });
            const blob = new Blob([file], { type: mimeType });
            ...
            if (!compressedBlob)
                throw new Error("Canvas export failed: Resulting blob is null");
            const result = await compressedBlob.arrayBuffer();
            const bytes = new Uint8Array(result);
            if (onProgress)
                onProgress({ percent: 100, message: "Done!" });
    ...
    async extractColorPalette(file, k = 5, onProgress) {
        ...
        try {
            ...
            if (onProgress)
                onProgress({ percent: 30, message: "Sampling pixels..." });
            const pixels = [];
            ...
            if (pixels.length === 0)
                return [];
            if (onProgress)
                onProgress({ percent: 50, message: "Clustering colors (k-means)..." });
            // k-means implementation - Select unique initial centroids
            // Sample up to 1000 pixels to find unique colors for initialization
            const sampleSize = Math.min(pixels.length, 1000);
            const sampledUnique = new Map();
            for (let i = 0; i < sampleSize; i++) {
                const p = pixels[Math.floor(Math.random() * pixels.length)];
                sampledUnique.set(p.join(','), p);
                if (sampledUnique.size >= k * 5)
                    break; // get enough candidates
            }
            ...
            for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
                ...
                for (let i = 0; i < pixels.length; i++) {
                    ...
                    let best_centroid = 0;
                    for (let j = 0; j < k; j++) {
                        const dist = Math.sqrt((pixels[i][0] - centroids[j][0]) ** 2 +
                ...
                // Update centroids
                const new_centroids = new Array(k).fill(0).map(() => [0, 0, 0]);
            ...
            if (onProgress)
                onProgress({ percent: 90, message: "Finalizing palette..." });
    ...
    async minifyCode(code, lang, onProgress) {
        ...
        let minified = code;
        if (lang === "css")
            minified = code.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\s*([{}:;,>~+])\s*/g, "$1").replace(/;\s*}/g, "}").replace(/\s+/g, " ").trim();
        if (lang === "html")
            minified = code.replace(/<!--[\s\S]*?-->/g, "").replace(/\s+/g, " ").replace(/>\s+</g, "><").trim();
        return { code: minified, error: null };
    },
    async computeDiff(textA, textB, ignoreWs = false, onProgress) {
    ...
    async encodeMp3(left, right, sampleRate, onProgress) {
        ...
        for (let i = 0; i < leftInt.length; i += sampleBlockSize) {
            ...
            if (mp3buf.length > 0)
                mp3Data.push(mp3buf);
            if (onProgress)
                onProgress({ percent: (i / left.length) * 100 });
        }
    ...
    async encodeWav(channels, sampleRate, onProgress) {
        if (onProgress)
            onProgress({ percent: 10, message: "Initializing WAV encoding..." });
        ...
        if (onProgress)
            onProgress({ percent: 30, message: "Encoding channels..." });
        const totalSamples = channels[0].length;
        while (pos < length) {
            if (onProgress && offset % 100000 === 0)
                onProgress({ percent: 30 + (offset / totalSamples) * 70, message: "Encoding..." });
    ...
    async createGif(frames, width, height, delay, onProgress) {
        ...
        for (let i = 0; i < frames.length; i++) {
            ...
            if (onProgress)
                onProgress({ percent: (i / frames.length) * 100 });
        }
        gif.finish();
        return gif.bytes();
    },
                ...
                if (fns[i] === OPS.paintImageXObject || fns[i] === OPS.paintImageXObjectRepeat) {
                    ...
                    try {
                        const imgData = await new Promise((res, rej) => {
                            page.objs.get(imgName, (img) => img ? res(img) : rej(new Error("not found")));
                        });
    ...
    async generateDocxFromText(text, onProgress) {
        if (onProgress)
            onProgress({ percent: 10, message: "Loading docx library..." });
        const { Document, Packer, Paragraph, TextRun } = await import("docx");
        if (onProgress)
            onProgress({ percent: 30, message: "Parsing text sections..." });
        ...
        if (onProgress)
            onProgress({ percent: 60, message: "Generating DOCX document..." });
        const doc = new Document({ sections });
        const blob = await Packer.toBlob(doc);
        if (onProgress)
            onProgress({ percent: 90, message: "Transferring binary..." });
        const buf = await blob.arrayBuffer();
        const outBytes = new Uint8Array(buf);
        return Comlink.transfer(outBytes, [outBytes.buffer]);
    },
    // Numeral Tasks
    async convertNumeral(input, inputFormat, targetFormat, extraOptions) {
    ...
    async detectNumeralFormat(input) {
        const { detectFormat } = await import("../features/numeral-converter/utils/conversion-helpers");
        return detectFormat(input);
    },
    async checkGrammar(text, ignoredWords, tone, onProgress) {
        if (onProgress)
            onProgress({ percent: 10, message: "Loading dictionaries..." });
    ...
    async parseLogs(logText, onProgress) {
        if (onProgress)
            onProgress({ percent: 10, message: "Splitting lines..." });
        const lines = logText.split("\n").filter((l) => l.trim().length > 0);
        const parsedLogs = [];
        const chunkSize = 5000;
        for (let i = 0; i < lines.length; i += chunkSize) {
            if (onProgress)
                onProgress({ percent: 10 + Math.floor((i / lines.length) * 85), message: `Parsing logs ${i} / ${lines.length}...` });
    ...
    async computePerceptualHash(file, mimeType, onProgress) { return ""; },
    async cropImageCenter(file, mimeType, width, height, onProgress) { return file; },
    async rotateImageStandard(file, mimeType, onProgress) { return file; },
    async generateSpriteSheet(file, mimeType, onProgress) { return file; },
    async optimizeSvg(file, mimeType, onProgress) { return ""; },
    async generateHistogram(file, mimeType, onProgress) { return []; },
    async simulateColorBlindness(fileOrData, mimeTypeOrType, typeOrProgress, onProgress) { return fileOrData; }
};
Comlink.expose(api);
