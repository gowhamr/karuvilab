...
const api = {
    ...
    async executeCanvasOperation(methodName, args) {
        ...
        const fn = canvasEngine[methodName];
        if (typeof fn !== 'function')
            throw new Error(`Unknown canvas operation: ${methodName}`);
    ...
    async resizeImage(file, width, height, mode, format, quality, onProgress) {
        let imgBitmap = null;
        try {
    ...
    async extractColorPalette(file, k = 5, onProgress) {
        ...
        try {
            ...
            for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
                ...
                if (!changed && iter > 0)
                    break; // converged
                const new_centroids = Array.from({ length: k }, () => [0, 0, 0]);
                ...
                if (onProgress)
                    onProgress({ percent: 50 + (iter / MAX_ITERATIONS) * 40, message: `Optimizing palette (iter ${iter + 1})...` });
            }
            if (onProgress)
                onProgress({ percent: 95, message: "Finalizing palette..." });
            ...
            for (let i = 0; i < sortedCentroids.length; i++) {
                ...
                for (let j = 0; j < uniqueColors.length; j++) {
                    ...
                    const dist = Math.sqrt((sortedCentroids[i][0] - uR) ** 2 +
