...
function getClientPurify() {
    ...
    try {
        ...
        if (_purifyInstance && !_hookInstalled && typeof _purifyInstance.addHook === 'function') {
            _hookInstalled = true;
            _purifyInstance.addHook('afterSanitizeAttributes', function (node) {
 ...
 */
export function parseMathNotation(text) {
    if (!text)
        return text;
    const cleanLatex = (math) => {
        return math
 ...
 */
export async function parseAndSanitizeMarkdown(md) {
 ...
 */
export function parseAndSanitizeMarkdownSync(md) {
    const mdWithMath = parseMathNotation(md);
    const rawHtml = marked.parse(mdWithMath, { async: false });
    return sanitizeHtml(rawHtml);
}
