 ...
 */
export function parseASN1(data, offset = 0) {
 ...
 */
export function decodeASN1String(bytes) {
    try {
 ...
 */
export function parseBinaryInput(input) {
...
export function formatPem(type, bytes) {
    const b64 = bytesToBase64(bytes);
    const lines = b64.match(/.{1,64}/g)?.join("\n") ?? b64;
    return `-----BEGIN ${type}-----\n${lines}\n-----END ${type}-----`;
}
export function parsePemBlocks(input) {
...
export function parseX509Certificate(der) {
    const root = parseASN1(der);
    if (root.children.length < 3)
        throw new Error("Invalid X.509 Certificate DER structure");
...
function parseDistinguishedName(node) {
    const result = {};
    if (!node.children)
        return result;
...
function formatDNString(dn) {
    return Object.entries(dn)
        .map(([k, v]) => `${k.replace(/\s*\([^)]*\)/, "")}=${v}`)
        .join(", ");
}
function parseASN1Time(bytes) {
