...
export function parseBERTLV(hexStr) {
    let offset = 0;
    const nodes = [];
    // Clean string
    ...
    while (offset < buffer.length) {
        ...
        if ((tagVal & 0x1F) === 0x1F) {
            do {
                ...
                if (offset >= buffer.length)
                    throw new Error("Unexpected end of data parsing tag");
        ...
        offset++;
        if (offset >= buffer.length)
            throw new Error(`Unexpected end of data after tag ${tagHex}`);
        // Parse length
        let len = 0;
        ...
        if ((lenFirst & 0x80) === 0) {
            ...
            if (lenBytes === 0)
                throw new Error("Indefinite length forms not supported in standard EMV");
            if (offset + lenBytes > buffer.length)
                throw new Error(`Unexpected end of data parsing length for tag ${tagHex}`);
