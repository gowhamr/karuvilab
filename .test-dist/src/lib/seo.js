 ...
 */
export function generateToolMetadata(toolId) {
 ...
 */
export function StructuredData({ tool, category, content: propsContent, isHead }) {
    const scripts = [];
    ...
    if (tool) {
        ...
        const getApplicationCategory = (catId) => {
            switch (catId) {
    ...
    return (_jsx(_Fragment, { children: scripts.map((s, i) => {
