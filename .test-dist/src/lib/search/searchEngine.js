        ...
        for (let j = 1; j <= a.length; j++) {
            ...
            if (row && prevRow) {
                if (b.charAt(i - 1) === a.charAt(j - 1)) {
                    ...
                    Math.min((row[j - 1] ?? 0) + 1, // insertion
...
export function searchTools(query, maxResults = 30) {
    ...
    for (const tool of index) {
        ...
        if (tool.keywords.some((k) => k.toLowerCase() === rawQuery)) {
            score += 200;
            if (!matchType)
                matchType = 'keyword';
        }
        else if (tool.keywords.some((k) => k.toLowerCase().startsWith(rawQuery))) {
            score += 100;
            if (!matchType)
                matchType = 'keyword';
        }
