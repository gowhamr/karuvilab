...
export function convertData(value, fromId, toId) {
...
export function calculateTransferTime(fileSize, fileUnitId, bandwidth, bandwidthUnitId, overheadPercent = 5) {
    const fromUnit = DATA_UNITS.find(u => u.id === fileUnitId);
    const bUnit = BANDWIDTH_UNITS.find(u => u.id === bandwidthUnitId);
    if (!fromUnit || !bUnit || bandwidth <= 0)
        return 0;
...
export function formatDuration(seconds) {
    ...
    if (m > 0)
        parts.push(`${m}m`);
    if (s > 0 || (parts.length === 0 && ms === 0))
        parts.push(`${s}s`);
    if (parts.length < 2 && ms > 0)
        parts.push(`${ms}ms`);
    return parts.join(' ');
}
export function calculateStorageCost(size, unitId, costPerGB, months) {
