...
export const useImageCompressStore = create((set, get) => ({
    items: [],
            ...
            if (ui.activeTab === 'single') {
                ...
                state.items.forEach(item => {
                    ...
                    if (item.compressedUrl)
                        blobManager.revoke(item.compressedUrl);
                });
    ...
    updateGlobalSettings: (settings) => {
        set((state) => ({
            ...
            items: state.items.map(item => item.status === 'idle' ? { ...item, settings: { ...item.settings, ...settings } } : item)
        }));
    },
    updateItemSettings: (id, settings) => {
        set((state) => ({
            items: state.items.map((item) => item.id === id ? { ...item, settings: { ...item.settings, ...settings }, status: 'idle' } : item),
        }));
    },
    ...
    compressItem: async (id) => {
        ...
        const result = await safeProcessor(async () => {
            ...
            const resultBytes = await batchCoordinator.enqueue(buffer, item.file.type, item.settings, (p) => {
                set((state) => ({
                    items: state.items.map((i) => (i.id === id ? { ...i, progress: p.percent } : i)),
                }));
            });
            const blob = new Blob([resultBytes], { type: item.settings.format });
                    ...
                    if (i.id === id) {
                        ...
                        if (nextItem.settings.strictPrivacyMode) {
                            if (nextItem.previewUrl)
                                blobManager.revoke(nextItem.previewUrl);
                            nextItem.previewUrl = '';
    ...
    reprocessItem: (id, replaceOriginal = false) => {
        ...
        if (!sourceItem || sourceItem.status !== 'completed' || !sourceItem.compressedBlob)
            return;
        ...
        set((state) => {
            ...
            if (replaceOriginal) {
                ...
                if (sourceItem.previewUrl)
                    blobManager.revoke(sourceItem.previewUrl);
                if (sourceItem.compressedUrl)
                    blobManager.revoke(sourceItem.compressedUrl);
