...
export async function runGrammarCheck(text, ignoredWords = [], tone = 'standard', onProgress) {
    ...
    const uniqueWordsSet = new Set(doc.terms().out('array').map((t) => t.toLowerCase().trim()).filter(Boolean));
