...
export async function runGrammarCheck(text, ignoredWords = [], tone = 'standard', onProgress) {
    if (!text.trim()) {
        return {
            errors: [],
            stats: {
    ...
    if (onProgress)
        onProgress({ percent: 30, message: "Loading dictionaries..." });
    const spell = await loadSpellchecker();
    if (onProgress)
        onProgress({ percent: 50, message: "Analyzing text..." });
    const rawErrors = [];
    const doc = nlp(text);
    // 1. Spelling Check
    ...
    while ((match = wordRegex.exec(text)) !== null) {
        ...
        if (word.toUpperCase() === word && word.length > 1)
            continue;
        if (lowerIgnored.has(word.toLowerCase()))
            continue;
    ...
    const svaPatterns = [
        { regex: /\b(he|she|it)\s+(are|were|have)\b/gi, fix: (m) => m[1] === 'have' ? `${m[0]} has` : `${m[0]} is` },
        { regex: /\b(they|we)\s+(is|was|has)\b/gi, fix: (m) => m[1] === 'has' ? `${m[0]} have` : `${m[0]} are` },
        { regex: /\b(I)\s+(is|has|are)\b/gi, fix: (m) => m[1] === 'has' ? 'I have' : 'I am' },
        { regex: /\b(you)\s+(is|was|has)\b/gi, fix: (m) => m[1] === 'has' ? 'you have' : 'you are' }
    ];
    ...
    if (onProgress)
        onProgress({ percent: 80, message: "Calculating readability..." });
    ...
    const readabilityScore = Math.max(0, Math.min(100, Math.round(206.835 - 1.015 * (words / sentences) - 84.6 * (totalSyllables / words))));
    ...
    const uniqueWordsSet = new Set(termsArray.map((t) => t.toLowerCase().trim()).filter(Boolean));
    ...
    if (readabilityScore >= 80)
        ...
        readabilityGrade = 'Standard (Grade 7-8)';
    else if (readabilityScore >= 40)
        readabilityGrade = 'Moderate (Grade 9-12)';
    else if (readabilityScore >= 20)
        readabilityGrade = 'Difficult (College)';
    ...
    for (const err of rawErrors) {
        ...
        if (!seenKeys.has(key)) {
            ...
            const uniqueReplacements = Array.from(new Set((err.replacements || [])
                .map(r => r.trim())
