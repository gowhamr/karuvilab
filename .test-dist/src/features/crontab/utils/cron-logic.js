/**
 * Cron Parser Engine
 * Handles validation, human-readable translation, and next-run prediction.
 */
export const MONTH_NAMES = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];
export const DAY_NAMES = ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'];
export const SPECIAL_EXPRESSIONS = {
    '@yearly': '0 0 1 1 *',
    '@annually': '0 0 1 1 *',
    '@monthly': '0 0 1 * *',
    '@weekly': '0 0 * * 0',
...
export function validateField(value, min, max) {
    ...
    if (value.includes('/')) {
        ...
        if (!range || !step || isNaN(Number(step)))
            return false;
...
export function fieldToHuman(value, type) {
    if (value === '*')
        return 'every ' + type.replace('dom', 'day').replace('dow', 'day of week');
    if (value.includes('/')) {
        const parts = value.split('/');
        const range = parts[0] || '*';
...
export function getValues(field, min, max) {
    if (!field)
        return [];
    if (field === '*') {
        const res = [];
...
export function getNextRuns(expr, count) {
    if (expr === 'REBOOT')
        return [];
...
export function parseCronExpression(expr) {
    if (!expr)
        return { valid: false, fields: [], humanReadable: '', nextRuns: [], error: 'Expression is empty' };
    if (expr === '@reboot') {
        return {
            valid: true,
            fields: [],
