...
export function MermaidDiagramBlock({ block }) {
    ...
    const diagramTypeLabel = preflight.type !== "unknown"
        ? preflight.type.charAt(0).toUpperCase() + preflight.type.slice(1)
        : "Mermaid";
    return (_jsx("div", { ref: containerRef, className: "mermaid-block-wrapper my-6 min-w-0 max-w-full", children: isLoading ? (_jsx(MermaidPlaceholder, { label: `Rendering ${diagramTypeLabel} diagram...` })) : result?.error ? (_jsx(MermaidErrorCard, { error: result.error, errorKind: result.errorKind, source: block.source, onRetry: () => renderDiagram(true) })) : result?.svg ? (_jsx(MermaidContainer, { id: block.id, svg: result.svg, source: block.source, diagramType: diagramTypeLabel, renderTimeMs: result.renderTimeMs, onRerender: () => renderDiagram(true) })) : null }));
}
