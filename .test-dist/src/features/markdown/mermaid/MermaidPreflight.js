...
export class MermaidPreflightAnalyzer {
     ...
     */
    static computeHash(source, theme = 'dark', configVersion = 'v1') {
     ...
     */
    static analyzeComplexity(source) {
        ...
        let complexity = 'low';
        let reason;
        if (characters > MERMAID_SECURITY_LIMITS.HARD_CAP_CHARS ||
            edges > MERMAID_SECURITY_LIMITS.HARD_CAP_EDGES) {
            complexity = 'blocked';
            reason = `Diagram exceeds hard limits (chars: ${characters}, edges: ${edges})`;
        }
        else if (characters > MERMAID_SECURITY_LIMITS.MAX_SOURCE_CHARS ||
