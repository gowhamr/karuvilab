...
export function tokenize(expr) {
    ...
    for (let i = 0; i < expr.length; i++) {
        ...
        if (/[0-9.]/.test(char)) {
            ...
            // Handle unary minus
            if (char === '-' && (tokens.length === 0 || tokens[tokens.length - 1] === '(' || tokens[tokens.length - 1] in OPERATORS)) {
                tokens.push('u-');
            }
            else {
                tokens.push(char);
        ...
        if (/[0-9]/.test(token) || token === 'pi' || token === 'e') {
            ...
            while (operators.length > 0 &&
                ...
                (FUNCTIONS.has(operators[operators.length - 1]) ||
                    (token !== 'u-' && OPERATORS[token] && OPERATORS[operators[operators.length - 1]]?.precedence > OPERATORS[token].precedence) ||
                    (token !== 'u-' && OPERATORS[token] && OPERATORS[operators[operators.length - 1]]?.precedence === OPERATORS[token].precedence && OPERATORS[token].associativity === 'L'))) {
                output.push(operators.pop());
            }
            operators.push(token);
        }
...
function convertAngle(value, unit, toRadians) {
...
export function evaluatePostfix(postfix, angleUnit) {
    if (postfix.length === 0)
        return '';
    const stack = [];
    try {
        for (const token of postfix) {
            if (token === 'pi') {
                ...
                switch (token) {
                    case 'sin':
                        stack.push(convertAngle(a, angleUnit, true).sin());
                        break;
                    case 'cos':
                        stack.push(convertAngle(a, angleUnit, true).cos());
                        break;
                    case 'tan':
                        stack.push(convertAngle(a, angleUnit, true).tan());
                        break;
                    case 'asin':
                        stack.push(convertAngle(a.asin(), angleUnit, false));
                        break;
                    case 'acos':
                        stack.push(convertAngle(a.acos(), angleUnit, false));
                        break;
                    case 'atan':
                        stack.push(convertAngle(a.atan(), angleUnit, false));
                        break;
                    case 'sinh':
                        stack.push(a.sinh());
                        break;
