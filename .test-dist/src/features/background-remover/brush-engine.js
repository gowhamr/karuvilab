...
export class BrushStudioEngine {
    ...
    historyIndex = -1;
    maxHistory = 20;
    constructor(canvas, originalImage) {
        this.canvas = canvas;
    ...
    saveHistoryState() {
        if (!this.ctx || typeof this.ctx.getImageData !== 'function') {
            ...
            this.historyStack.push({ width: this.canvas.width, height: this.canvas.height, data: new Uint8ClampedArray(4) });
     ...
     */
    paintStamp(x, y, mode, size, hardness, opacity) {
            ...
            if (typeof document !== 'undefined') {
                ...
                if (offCtx) {
                    if (typeof offCtx.drawImage === 'function' && this.originalImage) {
                        offCtx.drawImage(this.originalImage, x - size, y - size, size * 2, size * 2, 0, 0, size * 2, size * 2);
                    }
    ...
    toBlob(format = 'png', quality = 0.95) {
        return new Promise((resolve, reject) => {
            ...
            }, format === 'webp' ? 'image/webp' : 'image/png', quality);
        });
    }
}
