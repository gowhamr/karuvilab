 ...
 */
export function detectEpochUnit(rawInput) {
 ...
 */
export function normalizeToMilliseconds(rawInput, forcedUnit) {
 ...
 */
export function formatRelativeTime(date, nowMs = Date.now()) {
 ...
 */
export function parseEpoch(rawInput, forcedUnit, nowMs = Date.now()) {
    const str = String(rawInput).trim();
    if (!str)
        return null;
 ...
 */
export function convertDateToEpoch(dateStr, timeStr = '00:00:00', timezoneId = 'UTC') {
    if (!dateStr)
        return null;
    try {
...
export function calculateEpochOffset(baseEpochSeconds, operation, amount, unit) {
...
export function parseBatchTimestamps(rawText) {
 ...
 * Export Batch to CSV
 */
export function exportBatchToCsv(rows) {
