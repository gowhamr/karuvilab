 ...
 */
export function computeReactionStats(attempts, falseStartsCount = 0) {
    ...
    const medianReactionMs = attemptCount % 2 === 1 ? sorted[mid] : Math.round((sorted[mid - 1] + sorted[mid]) / 2);
    // Standard deviation & Consistency
    let stdDevMs = 0;
    let consistencyScore = 100;
    if (attemptCount >= 2) {
        const variance = attempts.reduce((acc, score) => acc + Math.pow(score - avgReactionMs, 2), 0) /
            (attemptCount - 1);
    ...
    const fastCount = attempts.filter((s) => s >= REACTION_BENCHMARKS.TOP_TIER_MAX_MS && s <= REACTION_BENCHMARKS.FAST_MAX_MS).length;
    const typicalCount = attempts.filter((s) => s > REACTION_BENCHMARKS.FAST_MAX_MS && s <= REACTION_BENCHMARKS.TYPICAL_MAX_MS).length;
