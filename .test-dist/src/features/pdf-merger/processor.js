...
const processor = {
    ...
    validate(input) {
        ...
        if (files.length < 2)
            return "Please provide at least 2 PDF files to merge.";
        if (!files.every(f => f instanceof File))
            return "Invalid file objects.";
        if (!files.every(f => f.type === "application/pdf"))
            return "All files must be PDFs.";
        return null;
    },
    async execute(input, options, signal, onProgress) {
        ...
        try {
            // Dispatch to worker
            const uint8Array = await workerOrchestrator.dispatch("mergePdfs", [arrayBuffers], arrayBuffers, // Transferables
            (p) => onProgress(20 + p.percent * 0.8), // Scale progress 20-100%
            signal, true, 2, 500, 60000);
            if (signal.aborted) {
                return {
                    status: "cancelled",
                    outputType: "download",
