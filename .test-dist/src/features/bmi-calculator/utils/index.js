...
export function calculateBMIPrime(bmi) {
    // BMI Prime = BMI / 25 (where 25 is upper limit of normal weight)
    return Number((bmi / 25.0).toFixed(2));
}
export function calculatePonderalIndex(weightKg, heightCm) {
...
export function calculateDeterministicBMI(input) {
    const { unit } = input;
    let heightCm;
    let weightKg;
    if (unit === 'imperial') {
...
export function calculateBMIResult(weightKg, heightCm, unit) {
