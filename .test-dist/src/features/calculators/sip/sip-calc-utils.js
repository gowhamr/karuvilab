 ...
 * FV = P * [((1 + i)^n - 1) / i] * (1 + i)
 */
export function calculateSipFutureValue(monthlyAmount, annualRatePercent, months) {
 ...
 */
export function calculateDeterministicSip(input) {
    const { monthlyInvestment, expectedAnnualReturn, timeHorizonYears, annualStepUpPercent = 0, lumpsumAmount = 0, annualInflationRate = 0, capitalGainsTaxRate = 0, expenseRatio = 0, } = input;
    ...
    if (monthlyInvestment > 100000000 ||
