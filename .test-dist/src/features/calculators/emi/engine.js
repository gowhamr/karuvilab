...
export function calculateEmi(principal, annualRatePercent, tenureMonths) {
    if (isNaN(principal) || isNaN(annualRatePercent) || isNaN(tenureMonths) || tenureMonths <= 0)
        return 0;
 ...
 */
export function calculateDeterministicEmi(input) {
    const { loanAmount, annualInterestRate, tenureMonths, prepayments = [], recurringPrepayment, moratorium, floatingRateDelta = 0, } = input;
    ...
    for (let y = 1; y <= totalYears; y++) {
        ...
        if (!firstMonthOfYear || !lastMonthOfYear)
            continue;
    ...
    let savings;
    const hasPrepayments = (prepayments && prepayments.length > 0 && prepayments.some(p => p.amount > 0)) ||
