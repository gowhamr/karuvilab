export function calculateYearProgress(asOfDate) {
    const startOfYear = new Date(asOfDate.getFullYear(), 0, 1);
    const endOfYear = new Date(asOfDate.getFullYear() + 1, 0, 1);
    return Math.min(100, Math.max(0, ((asOfDate.getTime() - startOfYear.getTime()) / (endOfYear.getTime() - startOfYear.getTime())) * 100));
}
...
export function calculateLifeStatistics(totalDays, totalHours, totalMinutes, asOfDate) {
