import { parseISODateParts, calculateCalendarDiff, getNextBirthdayPure, isLeapYear, daysFromCivil, getDayOfWeek, todayISO } from './date-utils';
import { calculateEphemeris } from './astronomy-utils';
import { getSunZodiacSign, getDetailedMoonPosition, getChineseZodiac, getBirthstoneAndFlower, } from './astrology-utils';
...
export function calculateFullAgeProfile(dob, asOf, showPrecisionTime = false, birthTime = "12:00", tzOffset = 0) {
    ...
    const moonDetails = getDetailedMoonPosition(core.dateOfBirth, showPrecisionTime ? birthTime : "12:00", showPrecisionTime ? tzOffset : 0);
    const ephemeris = calculateEphemeris(core.dateOfBirth, showPrecisionTime ? birthTime : "12:00", showPrecisionTime ? tzOffset : 0);
...
export function calculateAgeComparison(dob1, dob2) {
    if (!dob1 || !dob2)
        return null;
    ...
    if (!p1 || !p2)
        return null;
    const isD1Older = p1.year < p2.year ||
