...
export function runMonteCarloSimulation(inputs, trials = 1000, stdDevMultiplier = 1.0) {
    ...
    let successCount = 0;
    const terminalCorpuses = [];
    const depletionAges = [];
    for (let i = 0; i < trials; i++) {
        // Generate a return sequence for this trial
        const returnsSequence = new Array(yearsToLive);
...
export function runHistoricalBacktest(inputs, crisis) {
    const currentAge = inputs.currentAge ?? 30;
    const longevityAge = inputs.longevityAge ?? 85;
    const yearsToLive = Math.max(1, longevityAge - currentAge);
