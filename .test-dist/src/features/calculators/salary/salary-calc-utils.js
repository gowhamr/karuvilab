 ...
 */
export function calculateNewRegimeTax(taxableIncome) {
    ...
    let surchargeRate = 0;
    if (roundedTaxable > 20000000)
        surchargeRate = 0.25; // > 2 Cr
    else if (roundedTaxable > 10000000)
        surchargeRate = 0.15; // > 1 Cr
    else if (roundedTaxable > 5000000)
        surchargeRate = 0.10; // > 50 Lakh
 ...
 * Cess: 4% Health & Education Cess
 */
export function calculateOldRegimeTax(taxableIncome) {
    const roundedTaxable = Math.max(0, Math.round(taxableIncome));
    let unrebatedTax = 0;
    const slabs = [];
    ...
    let surchargeRate = 0;
    if (roundedTaxable > 50000000)
        surchargeRate = 0.37; // > 5 Cr
    else if (roundedTaxable > 20000000)
        surchargeRate = 0.25; // > 2 Cr
    else if (roundedTaxable > 10000000)
        surchargeRate = 0.15; // > 1 Cr
    else if (roundedTaxable > 5000000)
...
export function parseSalaryParamsFromUrl(searchParams) {
    const getParam = (key) => {
...
export function serializeSalaryParamsToUrl(input) {
